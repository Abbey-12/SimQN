...
class EntanglementDistributionApp(Application):
    ...
    def handle_response(self, packet: RecvClassicPacket):
        ...
        if cmd == "swap":
            ...
            # # Check fidelity after swapping
            # if new_epr.fidelity < fidelity_threshold:
            #     log.debug(f"{self.own}: fidelity {new_epr.fidelity} below threshold {self.fidelity_threshold}, marking as unsuccessful")
            #     classic_packet = ClassicPacket(
            #         msg={"cmd": "revoke", "transmit_id": transmit_id}, src=self.own, dest=from_node)
            #     cchannel.send(classic_packet, next_hop=from_node)
            #     log.debug(f"{self.own}: send {classic_packet.msg} to {from_node}")
            #     return
