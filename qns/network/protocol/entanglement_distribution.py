...
SECRET_KEY = b'my_secret_key_1234567890'
...
# enatanglement distribution app
...
class EntanglementDistributionApp(Application):
    ...
    def response_distribution(self, packet: RecvQubitPacket):
        ...
        # Altered classical channel communication
        # modified_msg = self.modify_message(packet.packet.get())
        ...
        if not ret1 or not ret2:
            ...
            # msg=modified_msg, src=self.own, dest=from_node)
    ...
    def modify_message(self, msg):
        # Simulate an attacker modifying the message
        modified_cmd = choice(["swap", "next", "succ", "revoke"])
        modified_msg = {"cmd": modified_cmd, "transmit_id": msg["transmit_id"]}
        return modified_msg
...
#             self.state[epr.transmit_id] = None
#         self.send_count += 1
#         self.request_distribution(epr.transmit_id)
...
#         ret2 = self.memory.write(next_epr)
#         if not ret1 or not ret2:
#             log.debug(f"{self.own}: store fail, destroy {epr} and {next_epr}")
#             # if failed (memory is full), destroy all entanglements
#             self.memory.read(epr)
#             self.memory.read(next_epr)
#             classic_packet = ClassicPacket(
#                 msg={"cmd": "revoke", "transmit_id": epr.transmit_id}, src=self.own, dest=from_node)
#             message = classic_packet.msg
#             signature = hmac.new(SECRET_KEY, str(message).encode(), digestmod='sha256').hexdigest()
...
#         expected_signature = hmac.new(SECRET_KEY, str(msg).encode(), digestmod='sha256').hexdigest()
...
#                     new_epr = first_epr.swapping(second_epr, name=uuid.uuid4().hex)
#                     log.debug(
#                         f"{self.own}: perform swap using {first_epr} and {second_epr}")
#                     log.debug(f"{self.own}: perform swap generate {new_epr}")
...
#                 classic_packet = ClassicPacket(
#                     msg={"cmd": "next", "transmit_id": transmit_id}, src=self.own, dest=from_node)
#                 self.send_with_authentication(classic_packet, from_node)
#             elif cmd == "next":
#                 # finish or request to the next hop
#                 if self.own == transmit.dst:
#                     result_epr = self.memory.read(transmit.first_epr_name)
...
#                     self.state[transmit_id] = None
#                     self.success_count += 1
#                     log.debug(f"{self.own}: successful distribution of {result_epr}")
...
#                         log.debug(
#                             f"{self.own}: send {classic_packet} to {from_node}")
#                         self.send_with_authentication(classic_packet, transmit.src)
#                 else:
#                     log.debug(f"{self.own}: begin new request {transmit_id}")
#                     self.request_distribution(transmit_id)
#             elif cmd == "succ":
#                 # the source notices that entanglement distribution is successful.
#                 result_epr = self.memory.read(transmit.second_epr_name)
#                 log.debug(f"{self.own}: received successful distribution {result_epr}")
#                 self.state[transmit_id] = None
#                 self.success_count += 1
#             elif cmd == "revoke":
#                 # clean memory
...
#                         self.send_with_authentication(classic_packet, transmit.src)
#         else:
#             raise Exception("Invalid signature! Potential tampering.")
