...
class EntanglementDistributionApp(Application):
    ...
    def RecvClassicPacketHandler(self, node: QNode, event: Event):
        self.receive_with_authentication(event)
    ...
    def response_distribution(self, packet: RecvQubitPacket):
        ...
        if not ret1 or not ret2:
            ...
            cchannel.send_with_authentication(classic_packet, next_hop=from_node)
    ...
    def send_with_authentication(self, classic_packet, next_hop):
        message = classic_packet.msg
        signature = hmac.new(SECRET_KEY, str(message).encode(), digestmod='sha256').hexdigest()
        message['signature'] = signature
        self.cchannel.send(classic_packet, next_hop=next_hop)

    def receive_with_authentication(self, packet):
        message = packet.packet.get()
        signature = message.pop('signature')
        expected_signature = hmac.new(SECRET_KEY, str(message).encode(), digestmod='sha256').hexdigest()
